...
public class Client extends JFrame implements ActionListener{
    ...
    //GUI 관련
    Container cp;
    JLabel label, westMidText;
    JPanel northPad, southPad, westWrap, westTopPad, westBotPad;
            ...
            public void keyPressed(KeyEvent ke){
                if(ke.getKeyCode() == KeyEvent.VK_ENTER){//엔터 누르면 실행
                    try {
                        ...
                        Thread th1 = new Receiver(sc); //서버로부터 수신받는 쓰레드 호출
                        th1.start();
        ...
        inputMsg.addKeyListener(new KeyAdapter(){
            public void keyPressed(KeyEvent ke2){
                if(ke2.getKeyCode()== KeyEvent.VK_ENTER){
                    ...
                    sender.sendMsg("//Chat" + msg); //내부클래스의 송신 메소드 호출
                    inputMsg.setText("");
                }
            }
        });
    ...
    void setPanel(){
        ...
        southPad = new JPanel();
        westWrap = new JPanel();
        westTopPad = new JPanel();
        westBotPad = new JPanel();
        westMidText = new JLabel("승패 여부");
        ...
        westWrap.setLayout(new GridLayout(3, 1));
        westWrap.add(westTopPad);
        westWrap.add(westMidText);
        westWrap.add(westBotPad);

        westBotPad.add(new JButton("뒷면"));
        westBotPad.add(new JButton("뒷면"));
        westBotPad.add(new JButton("뒷면"));
        westBotPad.add(new JButton("뒷면"));
        westBotPad.add(new JButton("뒷면"));

        westTopPad.add(new Label("뒷면"));
        westTopPad.add(new Label("뒷면"));
        westTopPad.add(new Label("뒷면"));
        westTopPad.add(new Label("뒷면"));
        westTopPad.add(new Label("뒷면"));
        ...
        cp.add(westWrap, BorderLayout.WEST);
    }
