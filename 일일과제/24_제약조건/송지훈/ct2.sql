...
alter table CT_DEPT add constraint CT_DEPT_CHK check (LOC=1 or LOC=2 );
...
alter table CT_EMP modify NAME constraint CT_EMP_난널 not null;
alter table CT_EMP add constraint CT_EMP_CHK check (ADDR='서울' or ADDR='부산' );
alter table CT_EMP add constraint CT_EMP_UNQ unique(JUMIN);
